...
const Profile = () => {
    ...
    const { user, order } = useAppSelector(store => store)
    ...
    useEffect(() => {
        if (user.profileUpdated || order.orderCanceled || user.error) {
            setOpenSnackbar(true);
        }
    }, [user.profileUpdated, order.orderCanceled]);
    return (
        <div className='px-5 lg:px-52 min-h-screen mt-10 '>
            ...
            >
                <Alert
                    ...
                    sx={{ width: "100%" }}
                >
                    {user.error ? user.error : order.orderCanceled ? "order canceled successfully" : "success"}
                </Alert>
            </Snackbar>
        </div>
    )
