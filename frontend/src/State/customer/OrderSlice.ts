...
export const fetchUserOrderHistory = createAsyncThunk<Order[], string>(
  ...
  async (jwt, { rejectWithValue }) => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.error || error.response?.data?.message || "Failed to fetch order history";
      if (error.response?.status === 403) {
        return rejectWithValue("Access forbidden. Please login again.");
      }
