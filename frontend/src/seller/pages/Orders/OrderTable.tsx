...
export default function OrderTable() {
  ...
  const handleClick = (event: any,orderId:number) => {
    setAnchorEl((prev:any)=>({
      ...prev, [orderId]:event.currentTarget
    }));
  };
            ...
            <StyledTableRow key={item.id}>
              ...
              <StyledTableCell>
                <div className='flex gap-1 flex-wrap'>
                 {item.orderItems.map((orderItem)=><div key={orderItem.product.id} className='flex gap-5 '>
                   <img 
                   ...
                   <div className='flex flex-col justify-between py-2'>
                    <h1 className='font-medium'>Title: {orderItem.product.title}</h1>
